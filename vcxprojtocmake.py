...
def main():
    ...
    try:
        ...
        ----------
        p : string
         .vcxproj file : Uncomment following lines to use this script without parameters
        """
...
def generate_cmake(tree, ns):
    ...
    try:
        ...
        for ref in references:
            ...
            cmakelist.write(
                '   add_subdirectory(${MAIN_DIR}/platform/cmake/' + os.path.splitext(path.basename(reference))[0] + ')\n')
        cmakelist.write('else()\n')
        for ref in references:
            reference = str(ref.get('Include'))
            cmakelist.write('   link_directories(${MAIN_DIR}/dependencies/' + os.path.splitext(path.basename(reference))[
